report 50031 "Order Confirmation - Bogala"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Reports/Order Confirmation Bogala.rdl';
    ...
    dataset
    {
        dataitem("Sales Header"; "Sales Header")
        {
            ...
            { }
            column(txtPurchaseComment; txtPurchaseComment)
            { }
            column(txtPurchaseCommentLine; txtPurchaseCommentLine)
            { }
            ...
            { }
            column(BankAccount_SWIFTCode; BankAccount_."SWIFT Code")
            { }
            column(BankAccountName; BankAccount_.Name)
            { }
            column(BankAccountAddress; BankAccount_.Address)
            { }
            column(BankAccountAddress2; BankAccount_."Address 2")
            { }
            column(RequestedDeliveryDate; "Requested Delivery Date")
            { }
            column(SalesHeaderSelltoPostCode; "Sales Header"."Sell-to Post Code")
            { }
            ...
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem(PageLoop; Integer)
                {
                    ...
                    { }
                    column(SalesHeaderShippingMarks; ShipMarks_)
                    { }
                    column(SalesHeaderPackingComm; "Sales Header"."Packing Comm.")
                    { }
                    ...
                    { }
                    column(SalesDestination; "Sales Header".Destination)
                    { }
                    column(PaymentTermsCode; "Sales Header"."Payment Terms Code")
                    { }
                    column(SalesLCNo; "Sales Header"."LC No")
                    { }
                    column(SalesLCDate; "Sales Header"."LC Date")
                    { }
                    column(SalesLCBank; "Sales Header"."LC Bank")
                    { }
                    ...
                    { }
                    column(CompanyInfoSvatNo; CompanyInfo."SVAT Registration No.")
                    { }
                    ...
                    { }
                    column(CustomerOrderNumber; "Sales Header"."Customer Order Number.")
                    { }
                    column(CustomerOrderDate; "Sales Header"."Customer Order Date")
                    { }
                    column(ShptDate_SalesHeader; FORMAT("Sales Header"."Shipment Date"))
                    { }
                    column(SalesHeaderTaxArea; "Sales Header"."Tax Area Code")
                    { }
                    ...
                    dataitem("Sales Line"; "Sales Line")
                    {
                        ...
                        column(compName; comRec.Name)
                        { }
                        column(comRecAddr; comRec.Address)
                        { }
                        column(country; CountryRegion.Name)
                        { }
                        ...
                        begin
                            comRec.Get;
                            CountryRegion.Get(comRec."Country/Region Code");
                        end;
                    }
                    dataitem(RoundLoop; Integer)
                    {
                        ...
                        { }
                        column(NoofPackages; "Sales Line"."No.of Packages")
                        { }
                        column(PackingComm; "Sales Line"."Packing Comm.")
                        { }
                        column(H_S_Code; "Sales Line"."H.S.Code")
                        { }
                        column(ShippingMarks; SalesLine."Shipping Marks")
                        { }
                        ...
                        begin
                            ...
                            NNCVATAmt := VATAmount;
                            //DP-2021-08-19-CAS-05410-R0C3 - changed NNCTotalInclVAT := NNCTotalLCY "-" NNCVATAmt;   to := NNCTotalLCY "+" NNCVATAmt;
                            ...
                            LineLevelTotal += SalesLine."Line Amount" - SalesLine."Inv. Discount Amount";

                            recTaxDetail.RESET;
                            recTaxDetail.SETRANGE(recTaxDetail."Tax Group Code", SalesLine."Tax Group Code");
                            ...
                            IF recTaxDetail.FIND('-') THEN BEGIN
                                REPEAT
                                    ...
                                    IF recTaxDetail."Tax Jurisdiction Code" = 'NBT' THEN BEGIN
                                        ...
                                        TotNBTAmt += (SalesLine."Line Amount" - SalesLine."Inv. Discount Amount") * taxBelowAmt / 100;
                                        //MESSAGE('Inside NBT Loop '+FORMAT(TotNBTAmt));
                                    END;

                                    IF recTaxDetail."Tax Jurisdiction Code" = 'SVAT' THEN BEGIN
                                        //LineLevelTotal+= (SalesLine."Line Amount"-SalesLine."Inv. Discount Amount")*taxBelowAmt/100;
                                        TotSVATAmt += (SalesLine."Line Amount" - SalesLine."Inv. Discount Amount" + TotNBTAmt) * taxBelowAmt / 100;
                                        //TotSVATAmt+=(SalesLine."Line Amount"-SalesLine."Inv. Discount Amount")*taxBelowAmt/100;
                                    END;

                                    IF recTaxDetail."Tax Jurisdiction Code" = 'VAT' THEN BEGIN
                                        LineLevelTotal += (SalesLine."Line Amount" - SalesLine."Inv. Discount Amount" + TotNBTAmt) * taxBelowAmt / 100;
                                        TotVATAmt += (SalesLine."Line Amount" - SalesLine."Inv. Discount Amount" + TotNBTAmt) * taxBelowAmt / 100;
                                        //MESSAGE('Inside VAT Loop');
                                    END;
                                    icount += 1;
                                    recTaxDetail.NEXT;
                            ...
                            recExtendedTextLine.SETRANGE(recExtendedTextLine."No.", SalesLine."No.");
                            REPEAT
            ...
            begin
                ...
                //==== CW-CustomerPhone=====
                recCustomer.RESET;
                recCustomer.SETRANGE(recCustomer."No.", "Sales Header"."Bill-to Customer No.");

                IF recCustomer.FINDFIRST THEN BEGIN

                    CountryRegion.RESET;
                    CountryRegion.SETRANGE(Code, recCustomer."Country/Region Code");
                    IF CountryRegion.FINDFIRST THEN;


                    CustomerPhone := recCustomer."Phone No.";
                END;
                //===================//

                recPurchaseCommentLine.RESET;
                recPurchaseCommentLine.SETRANGE(recPurchaseCommentLine."No.", "Sales Header"."No.");
                REPEAT
                    IF (recPurchaseCommentLine.Comment <> '') AND (recPurchaseCommentLine."Document Line No." = 0) THEN BEGIN
                        txtPurchaseComment += recPurchaseCommentLine.Comment + ' ';
                    END ELSE
                        IF (recPurchaseCommentLine.Comment <> '') AND (recPurchaseCommentLine."Document Line No." > 0) THEN BEGIN
                            txtPurchaseCommentLine += recPurchaseCommentLine.Comment + ' ';
                        END;
                    icount2 += 1;
                    recPurchaseCommentLine.NEXT;
                UNTIL recPurchaseCommentLine.COUNT < icount2;

                //JIT-SR01
                SalesLine_.RESET;
                SalesLine_.SETFILTER("Document No.", "No.");
                IF SalesLine_.FINDFIRST THEN
                    REPEAT
                        ShipMarks_ += SalesLine_."Shipping Marks" + ' ';
    ...
    var
        ...
        recPurchaseCommentLine: Record "Sales Comment Line";
        txtExtendedLine: Text[800];
        txtPurchaseComment: Text[800];
        ...
        icount1: Integer;
        txtPurchaseCommentLine: Text[300];
        txtDescriptionLine: Text[300];
        icount2: Integer;
        icount3: Integer;
        recPurchaseCommentLine1: Record "Purch. Comment Line";
