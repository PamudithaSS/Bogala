report 50008 "Check - GK Mines"
{
    // 0.01 22Aprl2013 - change CheckNo from DocNo to ExtDocNo
    //DefaultLayout = RDLC;
    //MR
    RDLCLayout = './Reports/Check-New.rdl';
    Caption = 'Check';
    ...
    dataset
    {
        ...
        dataitem(GenJnlLine; "Gen. Journal Line")
        {
            ...
            { }
            //RJ
            column(AccountPayeeOnly; AccountPayeeOnly)
            {
            }
            column(Valid60dys; Valid60dys)
            {
            }
            column(AccountPayeeText; AccountPayeeText)
            {
            }
            column(Valid60DaysText; Valid60DaysText)
            {
            }
            column(DocumentDate; GenJnlLine."Document Date")
            { }
            ...
            dataitem(CheckPages; Integer)
            {
                ...
                dataitem(PrintCheck; Integer)
                {
                    ...
                    column(InvNos; GenJnlLine."Applies-to Doc. No.")
                    {
                    }
                    ...
                    var
                        Decimals: Decimal;
                    begin
                        // IF AccountPayeeOnly = TRUE THEN BEGIN
                        //     AccountPayeeText := 'Account Payee Only';
                        // END ELSE BEGIN
                        //     AccountPayeeText := '';
                        // END;

                        // IF Valid60dys = TRUE THEN BEGIN
                        //     Valid60DaysText := 'Valid Only for 60 Days';
                        // END ELSE BEGIN
                        //     Valid60DaysText := '';
                        // END;
                ...
                begin
                    IF NOT TestPrint THEN BEGIN
                        IF UseCheckNo <> GenJnlLine."Document No." THEN BEGIN
                            ...
                            GenJnlLine3.SETRANGE("External Document No.", UseCheckNo);
                            // 0.01 <<
                            IF GenJnlLine3.FIND('-') THEN
                                // 0.01 >>
                                //GenJnlLine3.FIELDERROR("Document No.",STRSUBSTNO(Text013,UseCheckNo));
                                GenJnlLine3.FIELDERROR("External Document No.", STRSUBSTNO(Text013, UseCheckNo));
                            // 0.01 <<
                        END;
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    field(AccountPayeeOnly; AccountPayeeOnly)
                    {
                        ApplicationArea = All;
                        Caption = 'Account Payee Only';
                    }
                    field(Valid60dys; Valid60dys)
                    {
                        ApplicationArea = All;
                        Caption = 'Valid 60 Days';
                    }
                }
            }
        }
    ...
    var
        ...
        DescriptionLine: array[3] of Text[45];
        ...
        Text3: Text[30];
        Text4: Text[30];
        AccountPayeeText: Text[50];
        Valid60DaysText: Text[50];
        D1: Code[1];
        D2: Code[1];
        M1: Code[1];
        M2: Code[1];
    ...
    begin
        ...
        Tens_Cents := ((No * 100) MOD 100) DIV 10;
        Ones_Cents := (No * 100) MOD 10;

        //IF  Tens_Cents >= 2 THEN BEGIN
        //IF  (Tens_Cents >= 2) AND (Ones_Cents > 0) THEN BEGIN
        //AddToNoText(NoText,NoTextIndex,PrintExponent,TensText[Tens_Cents]+' ONLY**');
        //AddToNoText(NoText,NoTextIndex,PrintExponent,TensText[Tens_Cents]);
        //IF Ones_Cents > 0 THEN
        //AddToNoText(NoText,NoTextIndex,PrintExponent,OnesText[Ones_Cents]+' ONLY**');
        //ELSE IF Ones_Cents=0 THEN
        //AddToNoText(NoText,NoTextIndex,PrintExponent,TensText[Tens_Cents]+' ONLY**');
        //END ELSE
        //IF ((Tens_Cents * 10 + Ones_Cents) > 0)  THEN
        // AddToNoText(NoText,NoTextIndex,PrintExponent,OnesText[Tens_Cents * 10 + Ones_Cents]+' ONLY**')
        //IF ((Ones_Cents) > 0)  THEN
        //AddToNoText(NoText,NoTextIndex,PrintExponent,OnesText[Ones_Cents]+' ONLY**')
        // ELSE AddToNoText(NoText,NoTextIndex,PrintExponent,Text026+' ONLY**');

        IF (Tens_Cents >= 2) AND (Ones_Cents > 0) THEN
            AddToNoText(NoText, NoTextIndex, PrintExponent, TensText[Tens_Cents] + ' ' + OnesText[Ones_Cents] + ' ONLY**')
        ELSE
            IF (Tens_Cents >= 2) AND (Ones_Cents = 0) THEN
                AddToNoText(NoText, NoTextIndex, PrintExponent, TensText[Tens_Cents] + ' ONLY**')
            ELSE
                ...
                ELSE
                    IF (Tens_Cents < 2) AND (Ones_Cents = 0) THEN
                        AddToNoText(NoText, NoTextIndex, PrintExponent, Text026 + ' ONLY**');
        //When Tens_Cents=0
        //AddToNoText(NoText,NoTextIndex,PrintExponent,TensText[Tens_Cents * 10 + Ones_Cents]+' ONLY**');

        //Zero Only
        //AddToNoText(NoText,NoTextIndex,PrintExponent,Text026+' ONLY**');


        //IF  (Tens_Cents >= 2) THEN
        // AddToNoText(NoText,NoTextIndex,PrintExponent,OnesText[Tens_Cents * 10 + Ones_Cents]+' ONLY**')
        //ELSE IF (Tens_Cents >= 2) AND (Ones_Cents = 0) THEN
        //AddToNoText(NoText,NoTextIndex,PrintExponent,TensText[Tens_Cents]+' ONLY**')
        //ELSE IF (Tens_Cents < 2) AND (Ones_Cents > 0) THEN
        //AddToNoText(NoText,NoTextIndex,PrintExponent,OnesText[Ones_Cents]+' ONLY**')
        // ELSE AddToNoText(NoText,NoTextIndex,PrintExponent,Text026+' ONLY**');
